...
function findwordsMorePages($url)
{
    ...
    // array con key url e value findwords
    $multiText = [];
    ...
    foreach ($anchors as $element) {
        ...
        if (strpos($href, 'http')!==0 && strpos($href, '.html') > 0) {
            $path = $url . ltrim($href, '/');
            $content = getArticleContent($path);
            $multiText[$path] = findwords(htmlspecialchars_decode($content));
        }
    }
...
function insertData($text, $url, $results){
    $conn = connectToDatabase();

    $conn->query("INSERT INTO ricerca (url, risultati, testo) VALUES ('".$url."','".implode(" ", $results)."','".$text."'); ");
}

print_r(findwordsMorePages("http://www.cesenatoday.it/"));